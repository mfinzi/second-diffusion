...
for i in range(num_samples):
    ...
    image = Image.open("/scratch/yk2516/repos/diffusion_model/second-diffusion/stable_diffusion/burger.png")
    image_samples_i = np.array(image)
    ...
    # txt
    # file_path = os.path.join(sample_storage_path, 'eigs_StableDiffusion_more_iters.txt')
    file_path = os.path.join(sample_storage_path, 'eigs_StableDiffusion.txt')
    ...
    ax1.legend()
    ax1.set_xlabel("latent image -> eigenvalue indices (4*64*64=16384 in total)")
    ax1.set_ylabel("absolute values of eigenvalues")
    ax1.set_title(f"Stable Diffusion | Eigen-Spectrum of ∇^2_z log p(z) for samples {i}")
    ...
    # Save the figure
    plt.savefig(os.path.join(sample_storage_path, f'stable_diffusion_hessian_{i}.png'))
