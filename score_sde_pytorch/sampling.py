...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps, workdir=None, use_pred_cond=False):
    ...
    if sampler_name.lower() == 'ode':
        sampling_fn = get_ode_sampler(sde=sde, shape=shape, inverse_scaler=inverse_scaler,
                                      denoise=config.sampling.noise_removal, eps=eps, device=config.device)
    # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
    elif sampler_name.lower() == 'pc':
        predictor = get_predictor(config.sampling.predictor.lower())
        corrector = get_corrector(config.sampling.corrector.lower())
        sampling_fn = get_pc_sampler(sde=sde, shape=shape, predictor=predictor, corrector=corrector,
                                     inverse_scaler=inverse_scaler, snr=config.sampling.snr,
                                     n_steps=config.sampling.n_steps_each,
                                     probability_flow=config.sampling.probability_flow,
                                     continuous=config.training.continuous, denoise=config.sampling.noise_removal,
                                     eps=eps, device=config.device, workdir=workdir, use_pred_cond=use_pred_cond)
    else:
...
class LangevinCorrector(Corrector):
    ...
    def compute_hessian_eigvals(self, x, t, i):
        ...
        log_hessian_eigval_conditions = (i % 100 == 0) or (i in [
            925, 950, 975, 990, 991, 992, 993, 994, 995, 996, 997, 998, 999
        ])
    ...
    def update_fn(self, x, t):
        ...
        for i in range(n_steps):
            ...
            if self.use_pred_cond:
                ...
                x = x_mean + torch.sqrt(step_size * 2)[:, None, None, None] * \
                    (soln_sqrt @ noise.view(-1)).reshape(x.shape)
            else:
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr, n_steps=1, probability_flow=False,
                   continuous=False, denoise=True, eps=1e-3, device='cuda', workdir=None, use_pred_cond=False):
    ...
    predictor_update_fn = functools.partial(shared_predictor_update_fn, sde=sde, predictor=predictor,
                                            probability_flow=probability_flow, continuous=continuous, workdir=workdir,
                                            use_pred_cond=use_pred_cond)
    corrector_update_fn = functools.partial(shared_corrector_update_fn, sde=sde, corrector=corrector,
                                            continuous=continuous, snr=snr, n_steps=n_steps, workdir=workdir,
                                            use_pred_cond=use_pred_cond)
...
def get_ode_sampler(sde, shape, inverse_scaler, denoise=False, rtol=1e-5, atol=1e-5, method='RK45', eps=1e-3,
                    device='cuda'):
    ...
    def ode_sampler(model, z=None):
        ...
        with torch.no_grad():
            ...
            solution = integrate.solve_ivp(ode_func, (sde.T, eps), to_flattened_numpy(x), rtol=rtol, atol=atol,
                                           method=method)
            nfe = solution.nfev
